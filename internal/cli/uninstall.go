...
func promptUninstallConfirm(flags UninstallFlags, stdout io.Writer, stdin io.Reader) (bool, error) {
	if flags.All {
		_, _ = fmt.Fprintf(stdout, "This will remove %s managed configuration from all supported agents.\n", effectiveCommandName())
	} else {
		_, _ = fmt.Fprintf(stdout, "This will remove %s managed configuration from: %s\n", effectiveCommandName(), strings.Join(agentLabelsFromStrings(flags.Agents), ", "))
	}
