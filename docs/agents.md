...
| Copilot CLI     | `copilot-cli`    | Yes          | Yes | Full (Task tool)                 | No            | No             | `~/.copilot`                        |
...
| --------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| **Full (sub-agents)** | Each SDD phase runs in an isolated context window via native sub-agent delegation, package-managed subagents, or an OpenCode-compatible overlay. The orchestrator coordinates; sub-agents execute. | Claude Code, OpenCode, Kilo Code, Gemini CLI, Cursor, VS Code Copilot, Copilot CLI, Kimi Code, Kiro IDE, Qwen Code, Pi |
...
| Feature          | Claude Code | OpenCode | Kilo Code | Gemini CLI | Cursor | VS Code Copilot | Copilot CLI | Codex | Windsurf | Antigravity | Kiro IDE | Qwen Code | OpenClaw |   Pi    |
| ---------------- | :---------: | :------: | :-------: | :--------: | :----: | :-------------: | :---------: | :---: | :------: | :---------: | :------: | :-------: | :------: | :-----: |
...
- Uses Copilot CLI native tooling (`task`, `/model`, `/agents`, `/mcp`) for orchestration
- Skills at `~/.copilot/skills/`
- System prompt at `~/.copilot/copilot-instructions.md`
- MCP config at `~/.copilot/mcp.json`
- Shares `~/.copilot` with VS Code Copilot; only VS Code settings/prompts remain under the VS Code User profile
